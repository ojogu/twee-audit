...
class AuditService:
    ...
    def extract_tweets(self):
        try:
            logger.info(f"Loading tweets from JSON: {settings.tweet_json_path}")
            processed_tweets = self.json_parser.parse()

            if not processed_tweets:
                logger.warning("No tweets found in JSON to extract")
                return Result(success=True, count=0)

            logger.info(f"Parsed {len(processed_tweets)} tweets from JSON")
